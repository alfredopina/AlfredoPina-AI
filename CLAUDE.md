...
- **Nombre de archivo al descargar:** `uploadRecurso` ahora manda `blobContentDisposition` al subir (`attachment; filename="..."`, con variante `filename*=UTF-8''...` para acentos/ñ) — sin esto, como el archivo vive en `blob.core.windows.net` (dominio distinto al del sitio), el navegador ignoraba el atributo `download="..."` del link y usaba el nombre "seguro" interno del blob (con el UUID por delante). **Ojo:** esto solo aplica a archivos subidos DESPUÉS de este fix — los que ya estaban en Blob (ej. los de prueba `Manual X`/`Manual Y`) hay que volver a subirlos para que les quede el nombre bonito.
