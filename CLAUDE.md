...
**Admin (`/admin`), reorganizado sept-2026:** sidebar con grupos `TRACKING` (Comercial/Clientes/Grupos/Solicitudes) → `FORMATOS` (Diagnósticos/**Calificaciones**/Diplomas/Encuestas — Calificaciones construida 2026-09-21: alumnos nacen de un Grupo activo, `sql/022` corrido, **`sql/023` (reglas de evaluación: puntos/asistencias/frecuencias/proyecto → calificación y resultado automáticos) pendiente de correr por Alfredo**; "Crear Diplomas" ya se quitó, se reconstruye desde el grupo calificado, ver `CLAUDE_DETALLE.md` → "Calificaciones") → `PRODUCTOS` (Cursos/Recursos) → `HERRAMIENTAS` (Dashboard/Pendientes/Modelo). "Tracking View" es la landing por default (4 tarjetas-resumen). Notificaciones (campana + umbrales configurables urgente/seguimiento) construidas. Panel Modelo: visor interactivo de las 24 tablas / 30 relaciones del esquema completo.
