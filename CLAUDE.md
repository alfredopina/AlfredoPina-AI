...
- Primer módulo del sitio con una **Storage Account de Azure de verdad** — no existía ninguna antes (Agenda usa un link .ics, no Storage). Cuenta creada: **`apcwebrecursos`**, grupo `GR_AlfredoPina`, región Central US, Standard/LRS, tipo StorageV2. Contenedor Blob `recursos` (nivel de acceso "Blob" — acceso anónimo de solo lectura por archivo, no se puede listar el contenedor completo; requirió habilitar "Permitir el acceso anónimo en contenedores individuales" a nivel cuenta, que no viene activado por default). Dos tablas: `Cursos` (PartitionKey=herramienta, RowKey=curso id, propiedades `nombre`+`codigo`) y `Recursos` (PartitionKey=`<herramienta>_<cursoId>`, propiedades `tipo`/`titulo`/`texto`/`url`/`orden`). La cadena de conexión vive en la Application Setting `RECURSOS_STORAGE_CONNECTION` del Static Web App (en el portal nuevo esto vive bajo **"Variables de entorno"**, ya no bajo "Configuración" — cambió de nombre en la UI de Azure) — nunca en el repo, mismo patrón que `OUTLOOK_ICS_URL`.
- `api/getRecursos` (Function pública v3 clásico): recibe herramienta+curso+código por query string, valida contra `Cursos`, y si coincide regresa los recursos de `Recursos` agrupados por tipo. **Bug ya corregido:** la primera versión usaba `context.res.jsonBody`, que en el runtime de las "managed functions" de Static Web Apps no se serializa (regresaba status 200 con cuerpo vacío) — se corrigió usando `body` + `headers: {"Content-Type":"application/json"}` explícitos, igual que ya hacía `getAvailability`. Si se agrega otra Function nueva, usar ese mismo patrón desde el inicio.
- Hay una entidad de prueba real en la tabla `Cursos` (PartitionKey `excel`, RowKey `excel-bi`, código `EXCEL2026`) creada a mano desde el portal para confirmar el flujo — la tabla `Recursos` sigue vacía (sin contenido real todavía), así que ese curso desbloquea pero muestra las secciones vacías. Cuando exista el admin de Recursos, esta entidad de prueba se puede dejar o reemplazar con datos reales.
- Decisiones tomadas con Alfredo para esta fase:
  ...
  - **TODO — cosmético:** el nombre de prueba quedó como "Basico-Intermedio" sin acento en la tabla `Cursos` — corregir cuando se cargue contenido real o desde el futuro admin.
- **Pendiente para la siguiente sesión:** la parte de administración de Recursos — subir archivos (drag & drop a Blob), crear curso, reordenar (drag & drop), rotar código — en `admin/index.html` → sección Recursos, más las Functions `uploadRecurso`, `crearCurso`, `updateOrden`, `gestionarCodigo` (protegidas con `allowedRoles:["admin"]`, mismo patrón que ya protege `/admin/*`).
